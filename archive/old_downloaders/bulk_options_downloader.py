...
class ProgressTracker:
    ...
    def __init__(self, total_tasks: int):
        ...
        if DOWNLOAD_CONFIG.get("skip_existing_check", False):
            self.status = "🔄 DOWNLOADING"  # Simple mode: directly to downloading
        else:
    ...
    def update(self, increment: int = 1, file_size_mb: float = 0):
        ...
        if increment > 0:
            ...
            # In simple mode, always show downloading status
            if DOWNLOAD_CONFIG.get("skip_existing_check", False):
...
class ThetaBulkDownloader:
    ...
    def _load_download_history(self):
        ...
        # Check if we should skip existing file check for simple mode
        if DOWNLOAD_CONFIG.get("skip_existing_check", False):
            logger.info("Simple mode: Skipping existing file check - will download all requested dates")
    ...
    async def download_options_data(self, symbol: str, date: str, interval: str = "1m") -> bool:
        ...
        # Skip if already downloaded (unless in simple mode)
        if not DOWNLOAD_CONFIG.get("skip_existing_check", False) and file_key in self.downloaded_files:
    ...
    async def download_multiple_symbols(self, symbols: List[str], start_date: str, 
        ...
        # Skip resume status in simple mode
        if DOWNLOAD_CONFIG.get("skip_existing_check", False):
            print(f"🚀 Simple mode: Downloading all requested dates without checking existing files")
            # Create a simple file list for all dates
            all_files = []
            market_cal = MarketCalendar()
            for symbol in symbols:
                dates = market_cal.get_trading_days(start_date, end_date)
                for date in dates:
                    all_files.append({
                        'symbol': symbol,
                        'date': date,
        ...
        if not DOWNLOAD_CONFIG.get("skip_existing_check", False):
            if resume_status['can_resume']:
    ...
    async def download_backwards_with_boundary_detection(self, symbols: List[str], start_date: str, 
        ...
        # Simple mode: just try each date once and move on
        if DOWNLOAD_CONFIG.get("skip_existing_check", False):
            max_consecutive_failures = 1  # Don't retry failed dates
            chunk_size = DOWNLOAD_CONFIG.get('chunk_size', 10)
        else:
                ...
                for chunk_start in range(0, len(all_trading_days), chunk_size):
                    ...
                    for date in chunk_dates:
                        ...
                        if success:
                            ...
                            # Simple mode: just skip failed dates, no boundary detection
                            if DOWNLOAD_CONFIG.get("skip_existing_check", False):
                                continue  # Just move to next date
                    ...
                    # Simple mode: skip boundary detection, just move to next chunk
                    if DOWNLOAD_CONFIG.get("skip_existing_check", False):
...
async def main():
    ...
    try:
        ...
        async with ThetaBulkDownloader(
            ...
            if DOWNLOAD_CONFIG.get("skip_existing_check", False):
                # Simple mode - calculate total files from date range
                market_cal = MarketCalendar()
                dates = market_cal.get_trading_days(
                    DOWNLOAD_CONFIG['start_date'],
                    DOWNLOAD_CONFIG['end_date']
                )
                total_files = len(dates) * len(DOWNLOAD_CONFIG['symbols'])
                completed_files = 0
            else:
